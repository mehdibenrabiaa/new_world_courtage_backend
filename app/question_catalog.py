"""Hardcoded question catalogs — one list per questionnaire slug.

Each entry defines a question's *identity* (key, type, options) — none of
that is editable from the CRM. The CRM can only:
  - include/exclude an entry in a given questionnaire (add/remove), and
  - override its wording (question text, hint, placeholder) and its order.

Content here mirrors the vehicle/coverage questions already authored in the
site's own components/CarInsuranceForm.js DEFAULT_STEPS, minus whatever
VehicleIdentityForm already collects upstream (name, phone, permis date,
bonus-malus prefill, naissance, immat) so nothing is asked twice.
"""

CATALOGS: dict[str, list[dict]] = {
    "taxi": [
        {"key": "vehicle_choice", "section": "Véhicule", "type": "radio", "card": True,
         "eyebrow": "Pour commencer", "question": "Quel véhicule souhaitez-vous assurer ?",
         "options": [{"label": "Mon véhicule actuel", "value": "current"}, {"label": "Un futur achat", "value": "future"}]},
        {"key": "vehicle_type", "section": "Véhicule", "type": "radio", "card": True,
         "question": "Quel type de véhicule souhaitez-vous assurer ?",
         "options": [{"label": "Moto", "value": "moto"}, {"label": "Scooter", "value": "scooter"}, {"label": "Voiture", "value": "voiture"}]},
        {"key": "brand", "section": "Véhicule", "type": "input", "input_type": "text",
         "question": "Marque du véhicule", "placeholder": "Ex : Renault, Yamaha…"},
        {"key": "model", "section": "Véhicule", "type": "input", "input_type": "text",
         "question": "Modèle", "placeholder": "Ex : Clio, Série 3, CB500…"},
        {"key": "version", "section": "Véhicule", "type": "input", "input_type": "text",
         "question": "Version ou finition", "placeholder": "Ex : Sport, Confort, 1.5 dCi…"},
        {"key": "displacement", "section": "Véhicule", "type": "select", "question": "Cylindrée",
         "options": [
             {"label": "Moins de 50 cc", "value": "<50"}, {"label": "50 – 125 cc", "value": "50-125"},
             {"label": "126 – 250 cc", "value": "126-250"}, {"label": "251 – 500 cc", "value": "251-500"},
             {"label": "501 – 750 cc", "value": "501-750"}, {"label": "Plus de 750 cc", "value": ">750"},
         ]},
        {"key": "power", "section": "Véhicule", "type": "input", "input_type": "number",
         "question": "Puissance du moteur", "placeholder": "Puissance en kW ou CV"},
        {"key": "purchase_date", "section": "Véhicule", "type": "input", "input_type": "month",
         "question": "Date d'achat du véhicule"},
        {"key": "purchase_price", "section": "Véhicule", "type": "input", "input_type": "number",
         "question": "Prix d'achat", "placeholder": "Montant en €"},
        {"key": "first_registration", "section": "Véhicule", "type": "input", "input_type": "month",
         "question": "Première mise en circulation"},
        {"key": "condition", "section": "Véhicule", "type": "radio", "question": "Neuf ou d'occasion ?",
         "options": [{"label": "Neuf", "value": "neuf"}, {"label": "Occasion", "value": "occasion"}]},
        {"key": "usage", "section": "Véhicule", "type": "radio", "question": "Usage principal du véhicule",
         "options": [
             {"label": "Domicile – travail", "value": "commute"}, {"label": "Loisirs", "value": "leisure"},
             {"label": "Professionnel", "value": "professional"},
         ]},
        {"key": "mileage", "section": "Véhicule", "type": "select", "question": "Kilométrage annuel estimé",
         "options": [
             {"label": "Moins de 5 000 km", "value": "<5000"}, {"label": "5 000 – 10 000 km", "value": "5000-10000"},
             {"label": "10 000 – 15 000 km", "value": "10000-15000"}, {"label": "15 000 – 20 000 km", "value": "15000-20000"},
             {"label": "Plus de 20 000 km", "value": ">20000"},
         ]},
        {"key": "parking", "section": "Véhicule", "type": "radio", "question": "Stationnement la nuit",
         "options": [
             {"label": "Garage privé", "value": "garage"}, {"label": "Rue", "value": "street"},
             {"label": "Parking privé", "value": "private_parking"},
         ]},
        {"key": "antitheft", "section": "Véhicule", "type": "checkbox", "required": False,
         "hint": "Sélectionnez tout ce qui s'applique.", "question": "Dispositifs antivol",
         "options": [
             {"label": "Alarme", "value": "alarm"}, {"label": "Antivol mécanique", "value": "lock"},
             {"label": "Traceur GPS", "value": "tracker"},
         ]},
        {"key": "financing", "section": "Véhicule", "type": "radio", "question": "Financement du véhicule",
         "options": [
             {"label": "Comptant", "value": "owned"}, {"label": "Crédit", "value": "loan"},
             {"label": "Leasing", "value": "leasing"},
         ]},
        {"key": "age", "section": "Conducteur", "type": "select", "question": "Votre âge",
         "options": [
             {"label": "16 – 24 ans", "value": "16-24"}, {"label": "25 – 35 ans", "value": "25-35"},
             {"label": "36 – 45 ans", "value": "36-45"}, {"label": "46 – 55 ans", "value": "46-55"},
             {"label": "56 – 65 ans", "value": "56-65"}, {"label": "66 ans et plus", "value": "66+"},
         ]},
        {"key": "license_type", "section": "Conducteur", "type": "select", "question": "Type de permis",
         "options": [
             {"label": "Permis A – moto (> 35 kW)", "value": "A"}, {"label": "Permis A2 – moto limitée", "value": "A2"},
             {"label": "Permis A1 – 125 cc", "value": "A1"}, {"label": "Permis B – voiture", "value": "B"},
             {"label": "Permis AM – cyclomoteur", "value": "AM"},
         ]},
        {"key": "experience", "section": "Conducteur", "type": "select", "question": "Années d'expérience",
         "options": [
             {"label": "Moins d'1 an", "value": "<1"}, {"label": "1 – 3 ans", "value": "1-3"},
             {"label": "4 – 6 ans", "value": "4-6"}, {"label": "7 – 10 ans", "value": "7-10"},
             {"label": "11 – 20 ans", "value": "11-20"}, {"label": "Plus de 20 ans", "value": "20+"},
         ]},
        {"key": "primary_driver", "section": "Conducteur", "type": "radio", "question": "Êtes-vous le conducteur principal ?",
         "options": [{"label": "Conducteur principal", "value": "main"}, {"label": "Conducteur secondaire", "value": "secondary"}]},
        {"key": "additional_drivers", "section": "Conducteur", "type": "select", "question": "Conducteurs supplémentaires",
         "options": [
             {"label": "Aucun", "value": "0"}, {"label": "1 conducteur", "value": "1"},
             {"label": "2 conducteurs", "value": "2"}, {"label": "3 ou plus", "value": "3+"},
         ]},
        {"key": "claims_declared", "section": "Historique", "type": "radio",
         "question": "Sinistres déclarés ces 3 à 5 dernières années ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "claims_responsibility", "section": "Historique", "type": "radio",
         "question": "Ces sinistres étaient-ils de votre responsabilité ?",
         "options": [
             {"label": "Responsable", "value": "responsible"}, {"label": "Non responsable", "value": "not_responsible"},
             {"label": "Les deux", "value": "both"},
         ]},
        {"key": "bonus_malus", "section": "Historique", "type": "select", "question": "Coefficient bonus-malus",
         "options": [
             {"label": "0.50 — Bonus maximum", "value": "0.50"}, {"label": "0.51 – 0.79 — Bon conducteur", "value": "0.51-0.79"},
             {"label": "0.80 – 0.99 — Conducteur confirmé", "value": "0.80-0.99"}, {"label": "1.00 — Référence", "value": "1.00"},
             {"label": "1.01 – 1.25 — Malus léger", "value": "1.01-1.25"}, {"label": "1.26 – 2.00 — Malus modéré", "value": "1.26-2.00"},
             {"label": "2.01 – 3.50 — Malus élevé", "value": "2.01-3.50"},
         ]},
        {"key": "license_suspended", "section": "Historique", "type": "radio",
         "question": "Votre permis a-t-il déjà été suspendu ou annulé ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "coverage_type", "section": "Couverture", "type": "radio", "question": "Type de couverture souhaité",
         "options": [{"label": "Au tiers", "value": "third-party"}, {"label": "Tous risques", "value": "comprehensive"}]},
        {"key": "deductible", "section": "Couverture", "type": "select", "question": "Montant de franchise",
         "options": [
             {"label": "150 €", "value": "150"}, {"label": "300 €", "value": "300"}, {"label": "500 €", "value": "500"},
             {"label": "750 €", "value": "750"}, {"label": "1 000 €", "value": "1000"},
         ]},
        {"key": "legal_protection", "section": "Couverture", "type": "radio", "question": "Protection juridique ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "roadside_assistance", "section": "Couverture", "type": "radio", "question": "Assistance routière ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "replacement_vehicle", "section": "Couverture", "type": "radio",
         "question": "Véhicule de remplacement en cas de sinistre ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "effective_date", "section": "Contrat", "type": "input", "input_type": "date",
         "question": "Date de prise d'effet"},
        {"key": "payment_frequency", "section": "Contrat", "type": "radio", "question": "Fréquence de paiement",
         "options": [{"label": "Mensuelle", "value": "monthly"}, {"label": "Annuelle", "value": "yearly"}]},
        {"key": "preferences", "section": "Contrat", "type": "input", "input_type": "text", "required": False,
         "question": "Préférences ou contraintes", "placeholder": "Ex : assureur actuel, exclusions…"},
        {"key": "email", "section": "Contact", "type": "input", "input_type": "email",
         "question": "Adresse e-mail", "placeholder": "exemple@email.com"},
    ],
    # Transcribed from Formulaire_Garage_PROTECT_Remplissable.pdf, regrouped
    # into the 5 sections the site's step tabs should show (Coordonnées,
    # Risques, Antécédents, Flotte auto propre, Tarification) rather than the
    # PDF's own §-numbered groupings. That PDF's section numbering jumps from
    # 4 to 8 (no 5/6/7 in the pages provided) — this catalog only covers what
    # was in those pages, plus a placeholder Tarification pair (effective
    # date / payment frequency, mirroring taxi's contract questions) since
    # the PDF didn't include a pricing section at all — replace those two
    # once the real tarification fields are available. The vehicle-fleet
    # table (§10) is a repeating grid in the PDF but this catalog has no
    # repeating-row concept, so it's flattened into summary questions.
    "garage": [
        # — Produits — a gate screen shown before the step-by-step wizard,
        # not one of its steps/tabs (see "gate" below, handled by
        # CarInsuranceForm). Card-style, multiple choice.
        {"key": "produits_interesses", "section": "Produits", "type": "checkbox", "card": True, "gate": True,
         "question": "Quelles assurances vous intéressent ?",
         "options": [
             {"label": "Protect Garage", "value": "protect_garage"},
             {"label": "Les Convoyeurs", "value": "convoyeurs"},
             {"label": "Les Négociants", "value": "negociants"},
         ]},

        # — Coordonnées —
        {"key": "representant_legal", "section": "Coordonnées", "type": "input", "input_type": "text",
         "question": "Nom et prénom"},
        {"key": "mobile", "section": "Coordonnées", "type": "input", "input_type": "tel",
         "question": "Mobile"},
        {"key": "email_principal", "section": "Coordonnées", "type": "input", "input_type": "email",
         "question": "Email principal"},
        {"key": "siret", "section": "Coordonnées", "type": "input", "input_type": "number",
         "question": "SIRET"},
        {"key": "pct_detention_capital", "section": "Coordonnées", "type": "input", "input_type": "number", "unit": "percent",
         "question": "% détention du capital"},

        # — Risques —
        {"key": "activite_principale", "section": "Risques", "type": "checkbox", "products": ["protect_garage"],
         "question": "Activité principale",
         "options": [
             {"label": "Mécanicien réparateur automobile", "value": "mecanicien"},
             {"label": "Carrossier / tôlier", "value": "carrossier"},
             {"label": "Centre d'entretien automobile", "value": "centre_entretien"},
         ]},
        {"key": "activites_annexes", "section": "Risques", "type": "checkbox", "required": False, "products": ["protect_garage"],
         "question": "Activités annexes",
         "options": [
             {"label": "Carrossier / tôlier", "value": "carrossier"},
             {"label": "Centre d'entretien automobile", "value": "centre_entretien"},
             {"label": "Vente de véhicules dans la limite de 20% du CA", "value": "vente_vehicules"},
             {"label": "Location de véhicules <= 3,5 t et 7 places maximum, dans la limite de 10% du CA", "value": "location_vehicules"},
         ]},
        {"key": "effectif_total", "section": "Risques", "type": "input", "input_type": "number", "products": ["protect_garage"],
         "question": "Effectif total hors personnel administratif", "placeholder": "Ex : 10"},
        {"key": "surface_risque", "section": "Risques", "type": "input", "input_type": "number", "unit": "m2",
         "question": "Surface du risque en m²", "placeholder": "Ex : 150", "products": ["protect_garage"]},
        {"key": "nombre_vehicules_detenus", "section": "Risques", "type": "input", "input_type": "number",
         "question": "Nombre de véhicules détenus simultanément", "placeholder": "Ex : 5", "products": ["negociants"]},
        {"key": "montant_vehicule_plus_cher", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur",
         "question": "Montant du véhicule le plus cher", "placeholder": "Montant en €", "products": ["negociants"]},
        {"key": "valeur_moyenne_vehicule", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur",
         "question": "Valeur moyenne d'un véhicule", "placeholder": "Montant en €", "products": ["negociants"]},
        {"key": "convoyeur_age_25", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "L'âge du convoyeur et de ses préposés est-il supérieur à 25 ans ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_permis_2ans", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "Le convoyeur et ses préposés ont-ils plus de 2 ans de permis ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "redressement_judiciaire", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise est-elle en cours de redressement ou de liquidation judiciaire ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},

        # — Risques (Convoyeur) — transcribed from the real "Convoyeur automobile —
        # Formulaire de souscription" tool's Risque tab. Duplicated as its own
        # convoyeur-only entries rather than reusing/retagging the garage/négociant
        # versions (chiffre_affaires, effectif_total, …), per the no-sharing rule.
        {"key": "convoyeur_activite_principale", "section": "Risques", "type": "checkbox", "required": False, "products": ["convoyeurs"],
         "question": "Activité principale",
         "options": [{"label": "Convoyage de véhicule", "value": "convoyage_vehicule"}]},
        {"key": "convoyeur_risque_resilie", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "Le risque a-t-il été résilié par une autre compagnie ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "permis_suspendu_36mois", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "Suspension ou retrait de permis dans les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_chiffre_affaires", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur", "products": ["convoyeurs"],
         "question": "Chiffre d'affaires", "placeholder": "Montant en €"},
        {"key": "convoyeur_date_creation", "section": "Risques", "type": "input", "input_type": "date", "products": ["convoyeurs"],
         "question": "Date de création"},
        {"key": "convoyeur_deja_assuree", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise a-t-elle déjà été assurée ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_assurance_compagnie", "section": "Risques", "type": "input", "input_type": "text", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_deja_assuree", "question": "Compagnie d'assurance", "placeholder": "Ex : AXA, Allianz…",
         "skip_unless": {"key": "convoyeur_deja_assuree", "value": "oui"}},
        {"key": "convoyeur_assurance_depuis", "section": "Risques", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_deja_assuree", "question": "Depuis",
         "skip_unless": {"key": "convoyeur_deja_assuree", "value": "oui"}},
        {"key": "convoyeur_assurance_cotisation", "section": "Risques", "type": "input", "input_type": "number", "required": False, "unit": "eur", "products": ["convoyeurs"],
         "parent_key": "convoyeur_deja_assuree", "question": "Cotisation", "placeholder": "Montant en €",
         "skip_unless": {"key": "convoyeur_deja_assuree", "value": "oui"}},
        {"key": "convoyeur_date_resiliation", "section": "Risques", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_deja_assuree", "question": "Date de résiliation",
         "skip_unless": {"key": "convoyeur_deja_assuree", "value": "oui"}},
        {"key": "convoyeur_date_echeance", "section": "Risques", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_deja_assuree", "question": "Date échéance principale",
         "skip_unless": {"key": "convoyeur_deja_assuree", "value": "oui"}},
        {"key": "convoyeur_effectif_total", "section": "Risques", "type": "input", "input_type": "number", "products": ["convoyeurs"],
         "question": "Effectif total", "placeholder": "Ex : 10"},

        # "3 // Analyse des risques – Sinistralité" — its own eyebrow-headed
        # sub-block within Convoyeur's Risques tab (see CarInsuranceForm.js's
        # groupFieldsByEyebrow).
        {"key": "convoyeur_sinistres_36mois", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "eyebrow": "Sinistralité",
         "question": "Y a-t-il eu des sinistres sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_sinistres_responsables", "section": "Risques", "type": "input", "input_type": "number", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_sinistres_36mois", "question": "Nombre de sinistres responsables",
         "skip_unless": {"key": "convoyeur_sinistres_36mois", "value": "oui"}},
        {"key": "convoyeur_sinistres_non_responsables", "section": "Risques", "type": "input", "input_type": "number", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_sinistres_36mois", "question": "Nombre de sinistres non responsables",
         "skip_unless": {"key": "convoyeur_sinistres_36mois", "value": "oui"}},
        {"key": "convoyeur_charge_sinistre", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_sinistres_36mois", "question": "Charge sinistre",
         "skip_unless": {"key": "convoyeur_sinistres_36mois", "value": "oui"}},

        # "4 // Local et risques associés" — another eyebrow-headed sub-block.
        # redressement_judiciaire above is already convoyeur-tagged and reused
        # as-is rather than duplicated (identical wording in the real form).
        {"key": "convoyeur_local_dispose", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "eyebrow": "Local et risques associés",
         "question": "Votre client dispose-t-il d'un local ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_valeur_contenu", "section": "Risques", "type": "input", "input_type": "text", "unit": "eur", "required": False, "products": ["convoyeurs"],
         "question": "Valeur du contenu assuré"},
        {"key": "convoyeur_statut_immobilier", "section": "Risques", "type": "select", "required": False, "products": ["convoyeurs"],
         "question": "Statut immobilier",
         "skip_unless": {"key": "convoyeur_local_dispose", "value": "oui"},
         "options": [{"label": "Locataire", "value": "locataire"}, {"label": "Propriétaire", "value": "proprietaire"}]},
        {"key": "convoyeur_rc_auto", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise est-elle assurée en responsabilité civile Automobile ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_rc_professionnelle", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise est-elle assurée en responsabilité civile professionnelle ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_bien_assure", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "question": "Le bien est-il actuellement assuré ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_bien_compagnie", "section": "Risques", "type": "input", "input_type": "text", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_bien_assure", "question": "Compagnie", "placeholder": "Ex : AXA, Allianz…",
         "skip_unless": {"key": "convoyeur_bien_assure", "value": "oui"}},
        {"key": "convoyeur_bien_depuis", "section": "Risques", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_bien_assure", "question": "Depuis le",
         "skip_unless": {"key": "convoyeur_bien_assure", "value": "oui"}},
        {"key": "convoyeur_bien_echeance", "section": "Risques", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_bien_assure", "question": "Échéance",
         "skip_unless": {"key": "convoyeur_bien_assure", "value": "oui"}},
        {"key": "convoyeur_bien_sinistres_36mois", "section": "Risques", "type": "radio", "products": ["convoyeurs"],
         "parent_key": "convoyeur_bien_assure", "question": "Y a-t-il eu des sinistres sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "convoyeur_bien_nb_sinistres", "section": "Risques", "type": "input", "input_type": "number", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_bien_sinistres_36mois", "question": "Nombre de sinistres",
         "skip_unless": {"key": "convoyeur_bien_sinistres_36mois", "value": "oui"}},
        {"key": "convoyeur_bien_montant_sinistres", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur", "required": False, "products": ["convoyeurs"],
         "parent_key": "convoyeur_bien_sinistres_36mois", "question": "Montant total sinistres ouverts",
         "skip_unless": {"key": "convoyeur_bien_sinistres_36mois", "value": "oui"}},
        {"key": "convoyeur_observations", "section": "Risques", "type": "input", "input_type": "textarea", "required": False, "products": ["convoyeurs"],
         "question": "Observations"},

        # — Risques (Négociant) — transcribed from the real "Négociant" tool's
        # Risque tab (both pages). Own convoyeur-style duplicate entries, not
        # shared with the existing negociants fields above or with the other
        # products' equivalents.
        {"key": "negociant_activite_principale", "section": "Risques", "type": "input", "input_type": "text", "products": ["negociants"],
         "question": "Activité principale"},
        {"key": "negociant_activites_annexes", "section": "Risques", "type": "input", "input_type": "text", "required": False, "products": ["negociants"],
         "question": "Activités annexes"},
        {"key": "negociant_chiffre_affaires", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur", "products": ["negociants"],
         "question": "Chiffre d'affaires annuel", "placeholder": "Montant en €"},
        {"key": "negociant_type_bien", "section": "Risques", "type": "checkbox", "products": ["negociants"],
         "question": "Type de bien",
         "options": [{"label": "Local commercial", "value": "local_commercial"}, {"label": "Algéco / bureau", "value": "algeco_bureau"}]},
        {"key": "negociant_surface_risque", "section": "Risques", "type": "input", "input_type": "number", "unit": "m2", "products": ["negociants"],
         "question": "Surface totale du risque (m²)"},
        {"key": "negociant_statut_local", "section": "Risques", "type": "select", "products": ["negociants"],
         "question": "Statut du local",
         "options": [{"label": "Propriétaire", "value": "proprietaire"}, {"label": "Locataire", "value": "locataire"}]},
        {"key": "negociant_valeur_contenu", "section": "Risques", "type": "input", "input_type": "number", "unit": "eur", "products": ["negociants"],
         "question": "Valeur du contenu assuré (€)", "placeholder": "Montant en €"},
        {"key": "negociant_redressement_judiciaire", "section": "Risques", "type": "radio", "products": ["negociants"],
         "question": "L'entreprise est-elle en cours de redressement ou liquidation judiciaire ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "negociant_rc_auto", "section": "Risques", "type": "radio", "products": ["negociants"],
         "question": "L'entreprise est-elle assurée en responsabilité civile automobile ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "negociant_rc_professionnelle", "section": "Risques", "type": "radio", "products": ["negociants"],
         "question": "L'entreprise est-elle assurée en responsabilité civile professionnelle ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "negociant_bien_assure", "section": "Risques", "type": "radio", "products": ["negociants"],
         "question": "Le bien est-il actuellement assuré ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "negociant_bien_compagnie", "section": "Risques", "type": "input", "input_type": "text", "required": False, "products": ["negociants"],
         "parent_key": "negociant_bien_assure", "question": "Compagnie d'assurance", "placeholder": "Ex : AXA, Allianz…",
         "skip_unless": {"key": "negociant_bien_assure", "value": "oui"}},
        {"key": "negociant_bien_duree_assurance", "section": "Risques", "type": "input", "input_type": "text", "required": False, "products": ["negociants"],
         "parent_key": "negociant_bien_assure", "question": "Durée d'assurance",
         "skip_unless": {"key": "negociant_bien_assure", "value": "oui"}},
        {"key": "negociant_bien_date_echeance", "section": "Risques", "type": "input", "input_type": "date", "required": False, "products": ["negociants"],
         "parent_key": "negociant_bien_assure", "question": "Date d'échéance",
         "skip_unless": {"key": "negociant_bien_assure", "value": "oui"}},
        {"key": "negociant_sinistres_36mois", "section": "Risques", "type": "radio", "products": ["negociants"],
        "question": "Y a-t-il eu des sinistres sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "negociant_nb_sinistres", "section": "Risques", "type": "input", "input_type": "number", "required": False, "products": ["negociants"],
         "parent_key": "negociant_sinistres_36mois", "question": "Nombre de sinistres",
         "skip_unless": {"key": "negociant_sinistres_36mois", "value": "oui"}},
        {"key": "negociant_montant_sinistres", "section": "Risques", "type": "input", "input_type": "number", "required": False, "unit": "eur", "products": ["negociants"],
         "parent_key": "negociant_sinistres_36mois", "question": "Montant total des sinistres ouverts (€)",
         "skip_unless": {"key": "negociant_sinistres_36mois", "value": "oui"}},

        # — Antécédents — the plain déjà-créée/résiliée/déjà-assurée cluster
        # below is Convoyeur's own Antécédents flow (simpler than Garage's,
        # no hors-auto/auto split) — tagged convoyeurs-only rather than left
        # generic, now that Garage gets its own duplicate set right after.
        {"key": "date_creation", "section": "Antécédents", "type": "input", "input_type": "date", "products": ["convoyeurs"],
         "question": "Date de création"},
        {"key": "entreprise_deja_creee", "section": "Antécédents", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise est-elle déjà créée ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "entreprise_resiliee", "section": "Antécédents", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise a-t-elle été résiliée par son assureur ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "motif_resiliation", "section": "Antécédents", "type": "input", "input_type": "text", "required": False, "products": ["convoyeurs"],
         "question": "Motif de résiliation",
         "skip_unless": {"key": "entreprise_resiliee", "value": "oui"}},
        {"key": "motif_resiliation_date", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "question": "Date de résiliation",
         "skip_unless": {"key": "entreprise_resiliee", "value": "oui"}},
        {"key": "deja_assuree_36_mois", "section": "Antécédents", "type": "radio", "products": ["convoyeurs"],
         "question": "L'entreprise a-t-elle déjà été assurée sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "compagnie_precedente", "section": "Antécédents", "type": "input", "input_type": "text", "required": False, "products": ["convoyeurs"],
         "question": "Compagnie", "placeholder": "Ex : AXA, Allianz…"},
        {"key": "assurance_depuis", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "question": "Depuis"},
        {"key": "assurance_date_resiliation", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "question": "Date résiliation"},
        {"key": "assurance_date_echeance", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["convoyeurs"],
         "question": "Date d'échéance"},

        # — Antécédents (Garage Protect) — transcribed from the real
        # "9 // Antécédents" tab. sinistres_hors_auto*/sinistres_auto* below
        # are already garage-only and reused as-is.
        {"key": "garage_entreprise_resiliee", "section": "Antécédents", "type": "radio", "products": ["protect_garage"],
         "question": "L'entreprise a-t-elle été résiliée par son assureur ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "garage_deja_assuree_36_mois", "section": "Antécédents", "type": "radio", "products": ["protect_garage"],
         "question": "L'entreprise a-t-elle déjà été assurée sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "garage_compagnie_precedente", "section": "Antécédents", "type": "input", "input_type": "text", "required": False, "products": ["protect_garage"],
         "parent_key": "garage_deja_assuree_36_mois", "question": "Compagnie d'assurance", "placeholder": "Ex : AXA, Allianz…",
         "skip_unless": {"key": "garage_deja_assuree_36_mois", "value": "oui"}},
        {"key": "garage_assurance_depuis", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["protect_garage"],
         "parent_key": "garage_deja_assuree_36_mois", "question": "Depuis",
         "skip_unless": {"key": "garage_deja_assuree_36_mois", "value": "oui"}},
        {"key": "garage_assurance_date_resiliation", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["protect_garage"],
         "parent_key": "garage_deja_assuree_36_mois", "question": "Date de résiliation",
         "skip_unless": {"key": "garage_deja_assuree_36_mois", "value": "oui"}},
        {"key": "garage_assurance_date_echeance", "section": "Antécédents", "type": "input", "input_type": "date", "required": False, "products": ["protect_garage"],
         "parent_key": "garage_deja_assuree_36_mois", "question": "Date d'échéance",
         "skip_unless": {"key": "garage_deja_assuree_36_mois", "value": "oui"}},
        {"key": "garage_sinistres_hors_auto_36_mois", "section": "Antécédents", "type": "radio", "products": ["protect_garage"],
         "question": "Y a-t-il eu des sinistres hors-auto sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "garage_nombre_sinistres_hors_auto", "section": "Antécédents", "type": "input", "input_type": "number", "required": False, "products": ["protect_garage"],
         "parent_key": "garage_sinistres_hors_auto_36_mois", "question": "Nombre de sinistres", "placeholder": "Ex : 2",
         "skip_unless": {"key": "garage_sinistres_hors_auto_36_mois", "value": "oui"}},
        {"key": "garage_montant_sinistres_hors_auto", "section": "Antécédents", "type": "input", "input_type": "number", "required": False, "unit": "eur", "products": ["protect_garage"],
         "parent_key": "garage_sinistres_hors_auto_36_mois", "question": "Montant / charge", "placeholder": "Montant en €",
         "skip_unless": {"key": "garage_sinistres_hors_auto_36_mois", "value": "oui"}},
        {"key": "garage_sinistres_auto_hors_bris_grele_36_mois", "section": "Antécédents", "type": "radio", "products": ["protect_garage"],
         "question": "Y a-t-il eu des sinistres auto hors bris de glace et grêle sur les 36 derniers mois ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "garage_nombre_sinistres_auto_hors_bris_grele", "section": "Antécédents", "type": "input", "input_type": "number", "required": False, "products": ["protect_garage"],
         "parent_key": "garage_sinistres_auto_hors_bris_grele_36_mois", "question": "Nombre de sinistres", "placeholder": "Ex : 2",
         "skip_unless": {"key": "garage_sinistres_auto_hors_bris_grele_36_mois", "value": "oui"}},
        {"key": "garage_montant_sinistres_auto_hors_bris_grele", "section": "Antécédents", "type": "input", "input_type": "number", "required": False, "unit": "eur", "products": ["protect_garage"],
         "parent_key": "garage_sinistres_auto_hors_bris_grele_36_mois", "question": "Montant / charge", "placeholder": "Montant en €",
         "skip_unless": {"key": "garage_sinistres_auto_hors_bris_grele_36_mois", "value": "oui"}},

        # — Flotte auto propre —
        {"key": "w_garage", "section": "Flotte auto propre", "type": "input", "input_type": "text",
         "question": "W Garage", "placeholder": "Ex : W123456", "products": ["protect_garage"]},
        {"key": "w_garage_nombre_vehicules", "section": "Flotte auto propre", "type": "input", "input_type": "number", "products": ["protect_garage"],
         "question": "Nombre de véhicules W Garage", "placeholder": "Ex : 3"},
        {"key": "w_garage_vehicules", "section": "Flotte auto propre", "type": "input", "input_type": "text", "required": False, "products": ["protect_garage"],
         "parent_key": "w_garage_nombre_vehicules", "question": "Mode d'achat et usage par véhicule W Garage",
         "options": [
             {"label": "Comptant", "value": "comptant"}, {"label": "LOA", "value": "loa"},
             {"label": "LLD", "value": "lld"}, {"label": "Crédit bancaire", "value": "credit_bancaire"},
         ]},
        {"key": "flotte_nombre_vehicules", "section": "Flotte auto propre", "type": "input", "input_type": "number", "products": ["protect_garage"],
         "question": "Nombre de véhicules dans la flotte", "placeholder": "Ex : 3"},
        {"key": "flotte_immatriculations", "section": "Flotte auto propre", "type": "input", "input_type": "text", "required": False, "products": ["protect_garage"],
         "parent_key": "flotte_nombre_vehicules", "question": "Immatriculations (carte grise) des véhicules",
         "placeholder": "Ex : AB-123-CD, EF-456-GH…"},

        # — Tarification — transcribed from the real Tarification tab of the
        # broker's subscription tool (jsGarage.tarification). date_echeance,
        # "Protection juridique socle de base" and "Producteur" were
        # previously excluded (auto-computed / fixed-disabled / internal
        # broker-staff field in the source tool) but are now included as
        # plain user-editable fields per request — none of the source tool's
        # special behavior (auto-compute, forced value, disabled state,
        # broker picklist) is replicated here.
        {"key": "effective_date", "section": "Tarification", "type": "input", "input_type": "date",
         "question": "Choisir la date d'effet prévisionnelle"},
        {"key": "date_echeance", "section": "Tarification", "type": "input", "input_type": "date",
         "question": "Date d'échéance"},
        {"key": "payment_frequency", "section": "Tarification", "type": "select",
         "question": "Périodicité",
         "options": [
             {"label": "Mensuelle", "value": "monthly"}, {"label": "Trimestrielle", "value": "quarterly"},
             {"label": "Semestrielle", "value": "biannual"}, {"label": "Annuelle", "value": "yearly"},
         ]},
        {"key": "payment_mode", "section": "Tarification", "type": "select",
         "question": "Mode de règlement",
         "options": [
             {"label": "Prélèvement automatique", "value": "direct_debit"},
             {"label": "Avis d'échéance", "value": "invoice"},
         ]},
        {"key": "pj_socle_base", "section": "Tarification", "type": "radio",
         "question": "Protection juridique socle de base (incluse)",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "pj_optionnelle", "section": "Tarification", "type": "radio",
         "question": "Souhaitez-vous la Protection Juridique Optionnelle « Fiscale, Sociale et Prud'homale » ?",
         "options": [{"label": "Oui", "value": "oui"}, {"label": "Non", "value": "non"}]},
        {"key": "choix_lci", "section": "Tarification", "type": "select",
         "question": "Choix de LCI pour les véhicules confiés ou destinés à la vente",
         "options": [
             {"label": "200 000 €", "value": "200000"}, {"label": "320 000 €", "value": "320000"},
             {"label": "Autre", "value": "autre"},
         ]},
        {"key": "exprimez_vous", "section": "Tarification", "type": "input", "input_type": "textarea", "required": False,
         "question": "Exprimez-vous", "placeholder": "Une précision à ajouter sur votre demande ?"},
        # No broker/producer list exists in this codebase to populate a real
        # dropdown (the source tool's "Producteur" select pulls from an
        # internal staff picklist) — plain text until that list is provided.
        {"key": "producteur", "section": "Tarification", "type": "input", "input_type": "text",
         "question": "Producteur", "hint": "Vous n'êtes pas dans la liste ? Cliquez-ici",
         "placeholder": "Producteur en charge du dossier"},
        {"key": "chiffre_affaires", "section": "Tarification", "type": "input", "input_type": "number", "unit": "eur",
         "question": "Chiffre d'affaires", "placeholder": "Montant en €", "products": ["convoyeurs"]},
        {"key": "formule_souhaitee", "section": "Tarification", "type": "input", "input_type": "text",
         "question": "Formule souhaitée", "products": ["negociants"]},
    ],
}


def get_catalog(template: str) -> list[dict]:
    return CATALOGS.get(template, [])


def get_catalog_entry(template: str, key: str) -> dict | None:
    return next((e for e in get_catalog(template) if e["key"] == key), None)
