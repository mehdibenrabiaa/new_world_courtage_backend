...
# Questionnaire *identity* (question types/options) is developer-only —
# app/question_catalog.py, edited in code and deployed. These admin routes
# only manage which catalog questions are live/ordered/reworded, but that's
# still deliberately not something any CRM role can be granted: not part of
# the configurable permissions matrix, superadmin-only, full stop. Only
# GET .../questions below (what the public devis form reads) stays open.
...
def list_questionnaires(db: Session = Depends(get_db), _user=Depends(require_superadmin)):
...
def create_questionnaire(payload: QuestionnaireCreate, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
...
def get_questionnaire(slug: str, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
    return _questionnaire_out(_get_questionnaire_or_404(slug, db))


@router.patch("/{slug}", response_model=QuestionnaireOut)
def update_questionnaire(slug: str, payload: QuestionnaireUpdate, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
...
def list_available_catalog_entries(slug: str, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
...
def add_question(slug: str, payload: QuestionAdd, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
    questionnaire = _get_questionnaire_or_404(slug, db)
...
def update_question_wording(question_id: int, payload: QuestionWordingUpdate, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
...
def remove_question(question_id: int, db: Session = Depends(get_db), _user=Depends(require_superadmin)):
    question = _get_question_or_404(question_id, db)
    db.delete(question)
    db.commit()
